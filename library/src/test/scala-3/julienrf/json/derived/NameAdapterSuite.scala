...
class NameAdapterSuite extends AnyFeatureSpec with Checkers {
  ...
  Feature("customize the casing for field names") {
    ...
    Scenario("product type") {
      ...
      given fooFormat: OFormat[Foo] = oformat(snakeAdapter(2))
    ...
    Scenario("recursive type") {
      ...
      implicit val arbitraryTree: Arbitrary[Tree] = {
        def atDepth(depth: Int): Gen[Tree] =
          if (depth < 3) {
            Gen.oneOf(
              arbitrary[String].map(Leaf.apply),
              for {
                lhs <- atDepth(depth + 1)
                rhs <- atDepth(depth + 1)
              } yield Node(lhs, rhs)
            )
          } else arbitrary[String].map(Leaf.apply)
      ...
      {
        lazy val treeReads: Reads[Tree] = flat.reads[Tree]((__ \ "type").read[String], snakeAdapter(0))
        lazy val treeWrites: OWrites[Tree] = flat.owrites((__ \ "type").write[String], snakeAdapter(0))
  ...
  def snakeAdapter(max: Int = 2) = new NameAdapter {
    ...
    def increment(v1: String) = this.synchronized {
      ...
      if (max >= 1 && nameMap(v1) > max) throw new RuntimeException(s"Snake conversion applied more than $max times to field: $v1")
    }
